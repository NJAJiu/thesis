...
some concepts. Some of them may not be clear at this point, we will discuss
...
\begin{enumerate}
    ...
    \item Jitter $\varphi$\\
        The root-mean-square (RMS) of a series of time offsets. It indicates 
...
device. As mentioned in Section~\ref{sec:adjusting_system_time}, the frequency of 
system clock may not be accurate. It also impacts the calculation here. We
...
offset versus delay collected over a 24-hr period''~\cite{clock_filter}. It shows
...
horizontal line which go through the point $(\delta_0, \theta_0)$, if the samples
...
process sends 8 packets. The interval between each packet is 2s. With the
