...
In an ideal situation, if we have $T_1$ and $T_0$, which are timestamps gotten
from both devices at the same instant, then we can get the offset $\theta$:
$$\theta = T_1 - T_0$$
...
\begin{enumerate}
    \item Reference Timestamp\\
        ...
        packet which is send from a client to a server. 
